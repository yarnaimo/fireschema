...
const schemaToRuleWithMeta = (t) => {
  return index_js_1.rules.and(
    `__validator_meta__(data)`,
    (0, exports._schemaToRule)()(t),
  )
}
exports.schemaToRuleWithMeta = schemaToRuleWithMeta
const _schemaToRule =
  (parent = null, key = 'data') =>
  (t) => {
    ...
    if (t instanceof zod_1.ZodUnion)
      return index_js_1.rules.or(
        ...t.options.map((0, exports._schemaToRule)(parent, key)),
      )
    if (t instanceof zod_1.ZodIntersection)
      return index_js_1.rules.and(
        ...[t._def.left, t._def.right].map(
          (0, exports._schemaToRule)(parent, key),
        ),
      )
    if (t instanceof zod_1.ZodOptional)
      return (0, exports._schemaToRule)(
        parent,
        key,
      )(zod_1.z.union([t.unwrap(), zod_1.z.undefined()]))
    if (t instanceof zod_1.ZodNullable)
      return (0, exports._schemaToRule)(
        parent,
        key,
      )(zod_1.z.union([t.unwrap(), zod_1.z.null()]))
    if (t instanceof zod_1.ZodAny) return `true`
    if (t instanceof zod_1.ZodUnknown) return `true`
    if (t instanceof zod_1.ZodUndefined) return `!("${key}" in ${parent})`
    if (t instanceof zod_1.ZodNull) return `${name} == null`
    if (t instanceof zod_1.ZodBoolean) return `${name} is bool`
    if (t instanceof zod_1.ZodLiteral)
      return `${name} == ${JSON.stringify(t.value)}`
    if (t instanceof SchemaType_js_1.ZodTimestamp) return `${name} is timestamp`
    if (t instanceof zod_1.ZodRecord) return `${name} is map`
    ...
    if (t instanceof zod_1.ZodObject) {
      return (
        name === 'data' ? index_js_1.rules.andMultiline : index_js_1.rules.and
      )(
        ...(0, lifts_1.P)(
