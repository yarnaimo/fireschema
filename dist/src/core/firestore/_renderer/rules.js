...
const renderRules = ($allow, model, pIndent) => {
  ...
  const functions = model
    ? (0, functions_js_1.renderFunctions)(
        (0, format_js_1.validatorDef)(
          'data',
          (0, transformer_js_1.schemaToRuleWithMeta)(model.schema),
          indent,
        ),
        pIndent,
      )
  ...
  const hasWriteRules = array.some(
    ([op]) => op in index_js_1.allowOptions.write,
  )
  const rulesStr = (0, lifts_1.P)(
    array,
      ...
      if (op in index_js_1.allowOptions.write && op !== 'delete') {
        return [
          ...
          index_js_2.rules.and(
            condition,
            (0, format_js_1.validatorCall)('request.resource.data'),
          ),
        ]
      }
      return [op, condition]
