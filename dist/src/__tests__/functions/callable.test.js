...
!(async () => {
    ...
    '_createUser', data_js_1.userDataJson);
});
test('call', async () => {
    const result = await functions_client_js_1.typedCaller.call('createUser', data_js_1.userDataJson);
...
test('call - nested', async () => {
    const result = await functions_client_js_1.typedCaller.call('nested-toUpperCase', { text: 'text' });
    expect(result).toEqual({
        data: { result: 'TEXT' },
    });
    if (result.error) {
