...
const nestedIntersection = zod_1.z.intersection(
  zod_1.z.union([
    zod_1.z.object({ a: zod_1.z.string() }),
    zod_1.z.object({ a: zod_1.z.number().int() }),
  ]),
...
test.each([
  [
    zod_1.z.union([zod_1.z.string(), zod_1.z.number().int()]),
  ...
  [
    zod_1.z.number().int().min(5).max(20),
    '(data is int && data >= 5 && data <= 20)',
  ],
