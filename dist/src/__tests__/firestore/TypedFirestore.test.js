...
describe('utils', () => {
    test('getCollectionOptionsByName', () => {
        expect((0, _object_js_1.getCollectionOptionsByName)(firestore_schema_js_1.firestoreModel.schemaOptions, 'posts')).toEqual([
            {
                loc: 'versions.users.posts',
                options: expect.objectContaining({ model: firestore_schema_js_1.PostModel }),
            },
        ]);
        expect((0, _object_js_1.getCollectionOptionsByName)(firestore_collections_js_1.firestoreModelWithDup.schemaOptions, 'posts')).toEqual([
            {
                loc: 'versions.users.posts',
                options: expect.objectContaining({ model: firestore_schema_js_1.PostModel }),
            },
...
for (const env of ['web', 'admin']) {
    ...
    const expectAnyTimestamp = env === 'web' ? firestore_js_1.expectAnyTimestampWeb : firestore_js_1.expectAnyTimestampAdmin;
    ...
    describe($env('refs'), () => {
        test('ref equality', () => {
            expect((0, _universal_js_1.refEqualUniv)(r.users.raw, (0, _universal_js_1.collectionUniv)(app, 'versions/v1/users'))).toBe(false);
            expect((0, _universal_js_1.refEqualUniv)(r.user.raw, (0, _universal_js_1.docFromRootUniv)(app, 'versions/v1/users/user'))).toBe(false);
            expect((0, _universal_js_1.refEqualUniv)(r.users.raw, r.v1.collection('users').raw)).toBe(true);
            expect((0, _universal_js_1.refEqualUniv)(r.user.raw, r.v1.collection('users').doc('user').raw)).toBe(true);
        });
        env === 'web' &&
            test('query equality', () => {
                expect((0, _universal_js_1.queryEqualUniv)((0, _universal_js_1.buildQueryUniv)(r.users.raw, (q) => [
                    q.where('age', '>=', 10),
                    q.where('age', '<', 20),
                ]), r.teenUsers.raw)).toBe(true);
                expect((0, _universal_js_1.queryEqualUniv)((0, _universal_js_1.buildQueryUniv)(r.usersGroup.raw, (q) => [
                    q.where('age', '>=', 10),
                    q.where('age', '<', 20),
    ...
    describe($env('class structure'), () => {
        ...
        test.each([
            ...
            for (const instance of instances) {
                ...
                expect((0, _universal_js_1.refEqualUniv)(instance.raw, firstInstance.raw)).toBe(true);
            }
        });
        test.each([
            {
        ...
        test.each([
            {
                ...
                getData: (0, type_extends_1.typeExtends)()(async (options) => r.typedFirestore.runTransaction(async (tt) => tt.getData(r.user, options))),
            },
            r.user,
            r.posts.parentDocument(),
            r.typedFirestore.wrapDocument(r.user.raw),
            ...
            for (const data of [getR, getDataR]) {
                (0, tsd_1.expectType)(data);
                expect((0, _universal_js_1.refEqualUniv)(data.ref.raw, r.user.raw)).toBe(true);
            }
        });
        test.each([
            {
                get: async () => r.typedFirestore.runTransaction(async (tt) => tt.get(r.post)),
                getData: (0, type_extends_1.typeExtends)()(async (options) => r.typedFirestore.runTransaction(async (tt) => tt.getData(r.post, options))),
            },
            r.post,
            r.typedFirestore.wrapDocument(r.post.raw),
            ...
            for (const data of [getR, getDataR]) {
                (0, tsd_1.expectType)(data);
                expect((0, _universal_js_1.refEqualUniv)(data.ref.raw, r.post.raw)).toBe(true);
            }
        });
        ...
        describe($env('write (test rules)'), () => {
            ...
            test('create user (fails due to invalid timestamp)', async () => {
                ...
                for (const data of [
                    ...
                    { ...r.userData, [index_js_1._createdAt]: localTs(), [index_js_1._updatedAt]: serverTs() },
                    { ...r.userData, [index_js_1._createdAt]: serverTs(), [index_js_1._updatedAt]: localTs() },
                ]) {
                    await (0, app_js_1.assertFails)(async () => (0, _universal_js_1.setDocUniv)(r.user.raw, data));
                }
                ...
                for (const data of [
                    { [index_js_1._createdAt]: serverTs() },
                    { [index_js_1._createdAt]: serverTs(), [index_js_1._updatedAt]: serverTs() },
                    { [index_js_1._updatedAt]: localTs() },
                ]) {
                    await (0, app_js_1.assertFails)(async () => (0, _universal_js_1.updateDocUniv)(r.user.raw, data));
                }
            ...
            test('create user (fails due to invalid data)', async () => {
                await (0, app_js_1.assertFails)(async () => r.user.create({
                    ...r.userData,
            ...
            test('create user (fails due to unauthed)', async () => {
                await (0, app_js_1.assertFails)(async () => ur.user.create(data_js_1.createUserData));
            });
            describe('write to non-existing doc', () => {
                test('setMerge fails', async () => {
                    await (0, app_js_1.assertFails)(async () => r.user.setMerge(data_js_1.createUserData));
                });
                test('update fails', async () => {
                    await (0, app_js_1.assertFails)(async () => r.user.update(data_js_1.createUserData));
                });
            });
            test('overwrite user fails', async () => {
                await r.user.create(data_js_1.createUserData);
                await r.user.update(data_js_1.createUserData);
                await (0, app_js_1.assertFails)(async () => r.user.create(data_js_1.createUserData));
            });
        });
    describe($env('write'), () => {
        test.each([
            ...
            await fn();
            const snapRaw = await (0, _universal_js_1.getDocUniv)((0, _universal_js_1.docUniv)(r.usersRaw, 'user'), undefined);
        ...
        describe($env('hooks'), () => {
            ...
            describe('useTypedDocument', () => {
                test('safeRef', async () => {
                    ...
                    updateRef();
                    const { result, rerender, waitForNextUpdate, unmount } = (0, react_hooks_1.renderHook)(() => (0, useTypedDocument_js_1.useTypedDoc)(ref));
                ...
                test.each([useTypedDocument_js_1.useTypedDoc, useTypedDocument_js_1.useTypedDocOnce])('without transformer %#', async (hook) => {
                    const { result, waitForNextUpdate, unmount } = (0, react_hooks_1.renderHook)(() => hook(r.user));
                    expect(result.current).toBe(undefined);
                    await waitForNextUpdate();
                    expect(result.current).toMatchObject({
                        error: undefined,
                    ...
                    expect((0, _universal_js_1.refEqualUniv)(result.current.snap.ref.raw, r.user.raw)).toBe(true);
                    (0, tsd_1.expectType)(result.current.data);
                    // @ts-expect-error: wrong data type
                    (0, tsd_1.expectType)(result.current.data);
                    unmount();
                });
                test.each([useTypedDocument_js_1.useTypedDoc, useTypedDocument_js_1.useTypedDocOnce])('with transformer %#', async (hook) => {
                    const { result, waitForNextUpdate, unmount } = (0, react_hooks_1.renderHook)(() => hook(r.user, {
            ...
            describe('useTypedCollection', () => {
                test('safeRef', async () => {
                    ...
                    updateRef();
                    const { result, rerender, waitForNextUpdate, unmount } = (0, react_hooks_1.renderHook)(() => (0, useTypedCollection_js_1.useTypedCollection)(r.users.select._teen(random)));
                    expect(result.current).toBe(undefined);
                    await waitForNextUpdate();
                    expect(result.current).toMatchObject({
                        error: undefined,
                ...
                test.each([useTypedCollection_js_1.useTypedCollection /** useTypedQueryOnce */])('without transformer %#', async (hook) => {
                    const { result, waitForNextUpdate, unmount } = (0, react_hooks_1.renderHook)(() => hook(r.users.select.teen()));
                    expect(result.current).toBe(undefined);
                    await waitForNextUpdate();
                    expect(result.current).toMatchObject({
                        error: undefined,
                    ...
                    expect((0, _universal_js_1.refEqualUniv)(result.current.snap.docs[0].ref.raw, r.user.raw)).toBe(true);
                    (0, tsd_1.expectType)(result.current.data[0]);
                    // @ts-expect-error: wrong data type
                    (0, tsd_1.expectType)(result.current.data[0]);
                    unmount();
                });
                test.each([useTypedCollection_js_1.useTypedCollection /** useTypedQueryOnce */])('with transformer %#', async (hook) => {
                    const { result, waitForNextUpdate, unmount } = (0, react_hooks_1.renderHook)(() => hook(r.users.select.teen(), {
                        transformer: (data, snap) => {
                            (0, tsd_1.expectType)(snap);
                            return data.name;
