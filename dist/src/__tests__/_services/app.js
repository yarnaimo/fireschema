...
const firestoreEmulatorUrl = (path) =>
  `http://${emulator_js_1.localhost}:${emulator_js_1.emulators.firestore.port}/emulator/v1/projects/${emulator_js_1.projectId}${path}`
beforeAll(async () => {
...
afterEach(async () => {
  await got_1.default.delete(
    firestoreEmulatorUrl('/databases/(default)/documents'),
  )
})
afterAll(async () => {
...
const getTestAppWeb = (uid) => {
  const app = (0, app_2.initializeApp)(
    { projectId: emulator_js_1.projectId },
    appName(),
  )
  const firestore = (0, firestore_2.getFirestore)(app)
  ;(0, firestore_2.connectFirestoreEmulator)(
    firestore,
    emulator_js_1.localhost,
    emulator_js_1.emulators.firestore.port,
    {
