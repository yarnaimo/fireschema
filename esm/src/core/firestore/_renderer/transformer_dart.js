...
export const _objectToClass = (t, className, objectNum, objects) => {
    let result = `class ${className} {`;
    for (const [key, _t] of Object.entries(t.shape)) {
        const [filedName, objectNum_, objects_] = _fieldToDart(_t, className, (name) => name, objectNum, objects);
...
export const _fieldToDart = (t, parentName, filedNameGen, objectNum, objects) => {
    if (t instanceof ZodOptional) {
        return _fieldToDart(t.unwrap(), parentName, (name) => `${filedNameGen(name)}?`, objectNum, objects);
    }
    ...
    if (t instanceof ZodArray) {
        return _fieldToDart(t.element, parentName, (name) => filedNameGen(`List<${name}>`), objectNum, objects);
    }
