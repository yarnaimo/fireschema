import { P } from 'lifts';
import { ZodAny, ZodArray, ZodBoolean, ZodIntersection, ZodLiteral, ZodNull, ZodNullable, ZodNumber, ZodObject, ZodOptional, ZodRecord, ZodString, ZodTuple, ZodUndefined, ZodUnion, ZodUnknown, z, } from 'zod';
import { is } from '../../../lib/type.js';
import { ZodTimestamp } from '../../types/SchemaType.js';
import { rules } from '../../utils/index.js';
export const schemaToRuleWithMeta = (t) => {
    return rules.and(`__validator_meta__(data)`, _schemaToRule()(t));
};
export const _schemaToRule = (parent = null, key = 'data') => (t) => {
    ...
    if (t instanceof ZodIntersection)
        return rules.and(...[t._def.left, t._def.right].map(_schemaToRule(parent, key)));
    ...
    if (t instanceof ZodString) {
        return rules.and(`${name} is string`, ...t._def.checks.flatMap((c) => {
            return c.kind === 'min'
                ? [`${name}.size() >= ${c.value}`]
                : c.kind === 'max'
                    ? [`${name}.size() <= ${c.value}`]
    ...
    if (t instanceof ZodNumber) {
        return rules.and(t.isInt ? `${name} is int` : `${name} is number`, ...t._def.checks.flatMap((c) => {
            return c.kind === 'min'
                ? [`${name} >= ${c.value}`]
                : c.kind === 'max'
                    ? [`${name} <= ${c.value}`]
    ...
    if (t instanceof ZodTuple) {
        return rules.and(`${name} is list`, ...t.items.map((_t, i) => {
            return _schemaToRule(name, i)(_t);
        }));
    }
    if (t instanceof ZodObject) {
        return (name === 'data' ? rules.andMultiline : rules.and)(...P(Object.entries(t.shape).map(([key, _t]) => {
